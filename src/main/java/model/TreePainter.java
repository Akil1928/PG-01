 ...
 * El Canvas se redimensiona automáticamente para mostrar todo el árbol.
 */
public class TreePainter {
    ...
    private static final double PADDING = 40;   // margen alrededor del árbol
    ...
    public void paint(Canvas canvas, RecursionEngine.CallNode root,
        ...
        // ── 1. Asignar posiciones relativas ────────────────────────────────
        double[] counter = {0};
        assignX(root, counter);

        // Calcular offset X para centrar desde el margen izquierdo
        double offsetX = PADDING + NODE_R;
        shiftX(root, offsetX, 0);

        // ── 2. Calcular dimensiones reales necesarias ───────────────────────
        double maxX = 0;
        double maxY = 0;
        ...
        double requiredWidth  = maxX + NODE_R + PADDING;
        double requiredHeight = maxY + NODE_R + PADDING;

        // ── 3. Redimensionar el Canvas para que quepa todo el árbol ────────
        canvas.setWidth(Math.max(requiredWidth, 400));
        canvas.setHeight(Math.max(requiredHeight, 300));
    ...
    private void shiftX(RecursionEngine.CallNode node, double offsetX, int depth) {
        double[] pos = positions.get(node);
        if (pos != null) {
            pos[0] += offsetX;
            pos[1]  = depth * V_GAP + NODE_R + PADDING;
        }
        for (RecursionEngine.CallNode child : node.children)
            shiftX(child, offsetX, depth + 1);
    }
    ...
    private void drawNodes(GraphicsContext gc, RecursionEngine.CallNode node,
        ...
        boolean isHighlight = !order.isEmpty() &&
                highlightStep > 0 &&
        ...
        else if (node.fromemo)                            fill = COL_MEMO;
